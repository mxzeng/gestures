...
TEST(Cr48ProfileSensorFilterInterpreterTest, TrackingIdMappingTest) {
  ...
  HardwareState hs[] = {
    { 0.000, 0, 0, 0, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.010, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.020, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },
    { 0.030, 0, 0, 0, &fs[1], 0, 0, 0, 0, 0.0 },
    { 0.040, 0, 2, 2, &fs[2], 0, 0, 0, 0, 0.0 },
    { 0.050, 0, 2, 2, &fs[4], 0, 0, 0, 0, 0.0 },
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, CorrectFingerPositionTest) {
  ...
  HardwareState hs[] = {
    { 0.000, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.010, 0, 2, 2, &fs[1], 0, 0, 0, 0, 0.0 },
    { 0.010, 0, 2, 2, &fs[3], 0, 0, 0, 0, 0.0 },
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, FingerCrossOverTest) {
  ...
  HardwareState hs[] = {
    { 0.000, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.000, 0, 2, 2, &fs[1], 0, 0, 0, 0, 0.0 },
    { 0.010, 0, 2, 2, &fs[3], 0, 0, 0, 0, 0.0 },
    { 0.020, 0, 2, 2, &fs[5], 0, 0, 0, 0, 0.0 },
    { 0.030, 0, 2, 2, &fs[7], 0, 0, 0, 0, 0.0 },
    { 0.040, 0, 2, 2, &fs[9], 0, 0, 0, 0, 0.0 },
    { 0.050, 0, 2, 2, &fs[11], 0, 0, 0, 0, 0.0 },
    // the finger pattern will be swapped
    { 0.060, 0, 2, 2, &fs[13], 0, 0, 0, 0, 0.0 },
    // to left-top-right-bottom and should
    { 0.060, 0, 2, 2, &fs[15], 0, 0, 0, 0, 0.0 },
...
TEST(Cr48ProfileSensorFilterInterpreterTest, ClipNonLinearAreaTest) {
  ...
  HardwareState hs[] = {
    { 0.00, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.02, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },
    { 0.04, 0, 1, 1, &fs[2], 0, 0, 0, 0, 0.0 },
    { 0.06, 0, 1, 1, &fs[3], 0, 0, 0, 0, 0.0 },
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, MovingFingerTest) {
  ...
  HardwareState hs[] = {
    { 0.00, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.02, 0, 2, 2, &fs[1], 0, 0, 0, 0, 0.0 },
  };

  HardwareState hs2[] = {
    { 0.00, 0, 1, 1, &fs2[0], 0, 0, 0, 0, 0.0 },
    { 0.02, 0, 2, 2, &fs2[1], 0, 0, 0, 0, 0.0 },
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, HistoryTest) {
  ...
  HardwareState hs[] = {
    ...
    { 0.500, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },
    { 0.525, 0, 2, 2, &fs[1], 0, 0, 0, 0, 0.0 },
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, TwoToOneJumpTest) {
  ...
  HardwareState hs[] = {
    { 0.500, 0, 2, 2, &fs[0], 0, 0, 0, 0, 0.0 }, // fs[0] and fs[1]
    { 0.525, 0, 1, 1, &fs[2], 0, 0, 0, 0, 0.0 },
    // fs[0] lifts, reported by fw w/ fs[1] tid
    { 0.550, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },
    // fw switches to reporting fs[1] position
    { 0.575, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 }, // fw continues to report fs[1]
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, OneToTwoJumpTest) {
  ...
  HardwareState hs[] = {
    // time, buttons, finger count, touch count, fingers
    { 167.663976, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },  // 0
    { 167.674964, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },  // 1
    { 167.700748, 0, 2, 2, &fs[2], 0, 0, 0, 0, 0.0 },  // 2
    { 167.724575, 0, 2, 2, &fs[4], 0, 0, 0, 0, 0.0 },  // 3
    { 167.749059, 0, 2, 2, &fs[6], 0, 0, 0, 0, 0.0 },  // 4
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, WarpOnSwapTest) {
  ...
  HardwareState hs[] = {
    // time, buttons, finger count, touch count, fingers
    { 76.673715, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },  // 0
    { 76.68619699999999, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },  // 1
    { 76.712234, 0, 2, 2, &fs[2], 0, 0, 0, 0, 0.0 },  // 2
    { 76.736775, 0, 2, 2, &fs[4], 0, 0, 0, 0, 0.0 },  // 3
    { 76.760614, 0, 2, 2, &fs[6], 0, 0, 0, 0, 0.0 },  // 4
    { 76.785092, 0, 2, 2, &fs[8], 0, 0, 0, 0, 0.0 },  // 5
    { 76.808168, 0, 1, 1, &fs[10], 0, 0, 0, 0, 0.0 },  // 6
    { 76.820516, 0, 1, 1, &fs[11], 0, 0, 0, 0, 0.0 },  // 7
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, SensorJumpTest) {
  ...
  HardwareState hs[] = {
    // time, buttons, finger count, touch count, fingers
    { 42004.644897, 0, 2, 2, &fs[0], 0, 0, 0, 0, 0.0 },  // 0
    { 42004.669832, 0, 2, 2, &fs[2], 0, 0, 0, 0, 0.0 },  // 1
    { 42004.689832, 0, 2, 2, &fs[4], 0, 0, 0, 0, 0.0 },  // 2
    { 42004.709832, 0, 2, 2, &fs[6], 0, 0, 0, 0, 0.0 },  // 3 (a sensor
                                                         //    jump report)
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, BigJumpTest) {
  ...
  HardwareState hs[] = {
    // time, buttons, finger count, touch count, fingers
    { 95268.194898, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },  // 0
    { 95268.203728, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },  // 1 (jump bottom-left)
    { 95268.216215, 0, 1, 1, &fs[2], 0, 0, 0, 0, 0.0 },  // 2
    { 95268.229053, 0, 1, 1, &fs[3], 0, 0, 0, 0, 0.0 },  // 3
    { 95268.24309, 0, 0, 0, NULL, 0, 0, 0, 0, 0.0 },     // 4

    { 95268.194898, 0, 1, 1, &fs[4], 0, 0, 0, 0, 0.0 },  // 5
    { 95268.203728, 0, 1, 1, &fs[5], 0, 0, 0, 0, 0.0 },  // 6  (jump up)
    { 95268.216215, 0, 1, 1, &fs[6], 0, 0, 0, 0, 0.0 },  // 7
    { 95268.229053, 0, 1, 1, &fs[7], 0, 0, 0, 0, 0.0 },  // 8
    { 95268.24309, 0, 0, 0, NULL, 0, 0, 0, 0, 0.0 },     // 9

    { 95268.194898, 0, 1, 1, &fs[8], 0, 0, 0, 0, 0.0 },  // 10
    { 95268.203728, 0, 1, 1, &fs[9], 0, 0, 0, 0, 0.0 },  // 11 (jump left)
    { 95268.216215, 0, 1, 1, &fs[10], 0, 0, 0, 0, 0.0 }, // 12 jump back, so the
                                                         // tracking id
    { 95268.229053, 0, 1, 1, &fs[11], 0, 0, 0, 0, 0.0 }, // 13 should not be
                                                         // changed
  };
...
TEST(Cr48ProfileSensorFilterInterpreterTest, FastMoveTest) {
  ...
  HardwareState hs[] = {
    // time, buttons, finger count, touch count, fingers
    { 95268.194898, 0, 1, 1, &fs[0], 0, 0, 0, 0, 0.0 },  // 0
    { 95268.203728, 0, 1, 1, &fs[1], 0, 0, 0, 0, 0.0 },  // 1
    { 95268.216215, 0, 1, 1, &fs[2], 0, 0, 0, 0, 0.0 },  // 2
    { 95268.229053, 0, 1, 1, &fs[3], 0, 0, 0, 0, 0.0 },  // 3
  };
